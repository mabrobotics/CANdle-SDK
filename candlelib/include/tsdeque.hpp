...
/** @brief  A templated *thread-safe* collection based on dequeue
            ...
            The various "emplace" operations are factorized by using the generic
"addData_protected". This generic asks for a concrete operation to use, which can be passed as a
lambda.
**/
template <typename T>
class TQueueConcurrent
{
  ...
  public:
    ...
    {
        addData_protected([&] { _collection.emplace_front(std::forward<Args>(args)...); });
    }
    ...
    {
        addData_protected([&] { _collection.emplace_back(std::forward<Args>(args)...); });
    }
  ...
  private:
    /** @brief  Protects the deque, calls the provided function and notifies the presence of new
    data
        @param  The concrete operation to be used. It MUST be an operation which will add data to
    the deque, as it will notify that new data are available!
    **/
    template <class F>
    ...
    std::condition_variable
        _condNewData;  ///< Condition used to notify that new data are available.
};