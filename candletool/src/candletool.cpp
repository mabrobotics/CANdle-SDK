...
void CandleTool::setupMotor(u16 id, const std::string& cfgPath, bool force)
{
    ...
    if (!getField(cfg, ini, "motor", "pole pairs", regs.motorPolePairs.value))
        /* add a field here only if you want to test it against limits form the candletool.ini
         * file
         */
        memcpy(regs.motorName.value,
               (cfg["motor"]["name"]).c_str(),
...
std::string CandleTool::clean(std::string s)
{
...
void CandleTool::testLatency(u16 id)
{
    log.warn("Not implemented, available with can open");
...
void CandleTool::registerRead(u16 id, u16 regAdress, u8 subIndex)
{
    MD md(id, m_candle);
...
void CandleTool::clearErrors(u16 id, const std::string& level)
{
    ...
    if (level == "error")
        ...
        md.clearErrors();  // TODO: implement clear warnings if it's necessary
    else
    {
        md.clearErrors();
    }
