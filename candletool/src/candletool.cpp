...
CandleTool::CandleTool(mab::Candle& candle) : m_candle(candle)
{
    ...
    void CandleTool::setupMotor(u16 id, const std::string& cfgPath, bool force)
    {
        ...
        if (md.writeRegisters(regs.outputEncoderCalibrationMode,
                              regs.outputEncoderCalibrationMode) != MD::Error_t::OK)
        {
    ...
    void CandleTool::setupReadConfig(u16 id, const std::string& cfgName)
    {
        ...
        readIni["profile"]["quick stop deceleration"] =
            floatToString(regs.quickStopDeceleration.value);
        ...
        if (regs.outputEncoder.value == 0)
            ...
            readIni["output encoder"]["output encoder"] =
                ui::encoderTypes[regs.outputEncoder.value];
            ...
            while (checkFile)
            {
                ...
                if (stat(saveConfigPath.c_str(), &st) == 0)
                {
                    if (!ui::getOverwriteMotorConfigConfirmation(configName))
                    {
                        ...
                        saveConfigPath =
                            saveConfigPath.substr(0, saveConfigPath.find_last_of("/") + 1) +
                            configName;
    ...
    void CandleTool::testMove(u16 id, f32 targetPosition)
    {
        ...
        for (f32 t = 0.f; t < 1.f; t += 0.01f)
        {
            ...
            log.info(
                "[%4d] Position: %4.2f, Velocity: %4.1f", id, md.getPosition(), md.getVelocity());
            usleep(30000);
        }
        m_candle.end();
    }

    void CandleTool::testMoveAbsolute(
        u16 id, f32 targetPos, f32 velLimit, f32 accLimit, f32 dccLimit)
    {
        if (!tryAddMD80(id))
            return;
    ...
    void CandleTool::updateCandle(const std::string& mabFilePath, bool noReset)
    {
        ...
        static void fillBrIni(
            BrakeResistor & br, mINI::INIStructure & rIni, std::string sectionName)
        {
            f32 temperatureLimit = 0.0f;
        ...
        static void fillIcIni(IsolatedConv & ic, mINI::INIStructure & rIni, std::string sectionName)
        {
            ...
            rIni[sectionName]["type"] =
                PdsModule::moduleType2String(moduleType_E::ISOLATED_CONVERTER);
        }
        ...
        void CandleTool::pdsStoreConfig(u16 id)
        {
            ...
            if (result != err_E::OK)
                log.error(
                    "PDS Configuration save error [ %u ] [ %s:%u ]", result, __FILE__, __LINE__);
        }
        ...
        void CandleTool::encoder(u16 id)
        {
            ...
            while (1)
            {
                log.info("[%4d] Position: %6.2f, Velocity: %6.1f",
                         id,
                         md.getPosition(),
                         md.getVelocity());
                usleep(100000);
        ...
        mab::CANdleBaudrate_E CandleTool::checkSpeedForId(u16 id)
        {
            std::initializer_list<mab::CANdleBaudrate_E> bauds = {
                mab::CANdleBaudrate_E::CAN_BAUD_1M,
