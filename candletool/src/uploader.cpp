...
#define PBWIDTH 60
    ...
    void FirmwareUploader::printProgress(double percentage)
    {
    ...
    FirmwareUploader::ERROR_E FirmwareUploader::flashDevice(bool directly)
    {
        ...
        log.success("Firmware upload complete!");
    ...
    bool FirmwareUploader::sendInitCmd()
    {
        ...
        *(uint32_t*)&txBuff[1] = M_BOOT_ADDRESS;
        ...
        log.info("Detecting bootloader mode...");

        candle.sendGenericFDCanFrame(m_currentId, 5, (const char*)txBuff, (char*)rxBuff, 50);

        if (strcmp("OK", (char*)rxBuff) == 0)
            log.success("Bootloader detected!");
        else
        {
            log.error("Bootloader mode could not be entered!");
            return false;
        }
        return true;
    }
    ...
    {
        log.debug("Sending enter page programming mode command... ");
        ...
        log.error("Sending page program command FAILED!");
        return false;
    }

    bool FirmwareUploader::sendPage()
    {
        log.debug("Sending page [ %u ]", m_currentPage);
    ...
    {
        log.debug("Sending boot command...");
        ...
        log.error("Error while sending boot command!");
        return false;
    }
}  // namespace mab
