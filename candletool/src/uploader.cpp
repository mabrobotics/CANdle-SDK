...
#define PBWIDTH 60
	...
	void printProgress(double percentage)
	{
		...
		printf("\r[CANDLE] %3d%% [%.*s%*s] Downloading... ", val, lpad, PBSTR, rpad, "");
	...
	bool FirmwareUploader::flashDevice(int id, bool directly)
	{
		...
		candle.configCandleBaudrate(mab::CAN_BAUD_1M);
	...
	bool FirmwareUploader::sendInitCmd()
	{
		uint8_t txBuff[64] = {0};
		char	rxBuff[64] = {0};
		...
		*(uint32_t*)&txBuff[1] = M_BOOT_ADDRESS;
		...
		{
			std::cout << "[CANDLE] Bootloader mode could not be entered!" << std::endl;
			return false;
		}
		return true;
	}

	bool FirmwareUploader::sendPageProgCmd()
	{
#ifdef UPLOAD_VERBOSE
		std::cout << "[DEBUG] Sending enter page programming mode command... ";
#endif
		...
		std::cout << std::endl << "[CANDLE] Error while sending page program command!";
		return false;
	}

	bool FirmwareUploader::sendPage()
	{
#ifdef UPLOAD_VERBOSE
	...
	bool FirmwareUploader::sendWriteCmd(uint8_t* pPageBuffer, int bufferSize)
	{
#ifdef UPLOAD_VERBOSE
		std::cout << "[DEBUG] Sending write command..." << std::endl;
#endif
		...
		*(uint32_t*)&txBuff[1] = CalcCRC(pPageBuffer, (uint32_t)bufferSize);
		...
		std::cout << std::endl << "[CANDLE] CRC fail!";
		return false;
	}

	bool FirmwareUploader::sendBootCmd()
	{
#ifdef UPLOAD_VERBOSE
		std::cout << "[DEBUG] Sending boot command..." << std::endl;
#endif
		...
		std::cout << std::endl << "[CANDLE] Error while sending boot command!" << std::endl;
		return false;
	}
}  // namespace mab
